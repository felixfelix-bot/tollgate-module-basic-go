// Package port holds the library-agnostic Cashu wallet contract used by
// TollGate: the WalletPort interface, the value types that cross the seam
// (Token, MintQuote, MeltQuote, MeltResult, MintQuoteState), and the sentinel
// errors every adapter must return so callers can match with errors.Is.
//
// WHY THIS IS A SEPARATE PACKAGE (T16, research/wallet-migration): the
// contract must be importable WITHOUT the concrete wallet library. Before the
// split, these declarations lived in package tollwallet, whose other files
// import gonuts-tollgate unconditionally — so anything importing the contract
// (including a conformance suite meant to run against a CDK or nucula adapter)
// transitively pulled gonuts-tollgate into its build. Here the package imports
// only the standard library, which is what makes "the backend is a choice"
// mechanically true rather than aspirational.
//
// package tollwallet re-exports every symbol in this package as a type alias,
// so existing callers (src/merchant) are unaffected.
//
// Design decisions documented in src/tollwallet/WIREFORMAT.md:
...
//   - Sentinels ErrTokenAlreadySpent, ErrLockedToken and ErrWalletNotInitialized
//     live here, not in the adapter, so every adapter returns the same values.
package port
...
// Sentinel errors of the wallet contract. Adapters MUST return these exact
// values (wrapped with %w is fine) rather than library-specific errors, so
// callers can use errors.Is without knowing which library is behind the port.
var (
	// ErrTokenAlreadySpent is returned (wrapped) when a mint rejects a token
	// because its secrets were already spent.
	ErrTokenAlreadySpent = errors.New("Token already spent")

	// ErrLockedToken is returned when a token carries spending conditions
	// (P2PK/HTLC) and cannot be spent by the gateway.
	ErrLockedToken = errors.New("token has spending conditions and cannot be spent by the gateway")

	// ErrWalletNotInitialized is returned by wallet operations when the
	// underlying Cashu wallet has not been initialized (for example on a bare
	// Merchant or in degraded mode), so callers get an error instead of a
	// nil-pointer panic.
...
// String returns the canonical uppercase string representation per the Cashu
...
// cleanup. The characterization test that pinned gonuts's behaviour lives in
// the library-specific test set.
...
// MarshalJSON implements json.Marshaler. Emits uppercase strings per the
// Cashu NUT-04 wire format. Value receiver so it works on both
...
// and the transitland-lib migration precedent (PR #640).
...
//
// Implementations:
//   - GonutsWallet (src/tollwallet/gonuts_wallet.go, build tag !cdk_wallet) —
//     wraps *TollWallet
//   - CdkWallet (src/tollwallet/cdk_wallet.go, build tag cdk_wallet) — wraps
//     cdk_ffi.Wallet via CGO FFI bindings
//
// It is consumed by merchant.Merchant via a WalletPort-typed field, which is
// what allows wallet injection in tests (see the conformance suite in
// src/tollwallet/conformance).
