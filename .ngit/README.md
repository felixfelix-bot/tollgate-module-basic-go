...
| `act/workflows/regression.yml` | The customer happy path, on the commit under test. Job `happy-path` stages an artifact from the checkout (module binary + the bundle from the pinned portal commit) and runs `tests/happy-path/run.sh` against it: module API contract, enforcement via the fake-`ndsctl` seam, and the guest SPA in a real browser (offline stub mint — no secret, no real ecash). Job `cloud-lab` runs the `tests/cloud-lab` docker-compose lab (real `cdk-mintd`, real binary, python client). Both call the same entry point a human runs — `scripts/regression-lane.sh`, via `make regression` — so the lane and the local command cannot drift. See `REGRESSION.md` (inventory) and `RELEASE-GATE.md` (what must be green before a feed bump). |
